...
char *createMessage(char *to, char *messageType, char *payload){
    char *buf;
    char *from = getDeviceName();
    int len = strlen(from)+ strlen(to)+strlen(messageType)+strlen(payload)+
              strlen("{type:prop,from:,to:,payload:{status:,message:}}");
    buf = calloc(1, len+1);
    if (buf) {
        snprintf(buf, len, "{type:prop,from:%s,to:%s,payload:{status:%s,message:%s}}", from, to, messageType, payload);
    }
    return buf;
}
