...
void generateCertificateRequest(WebinosJSON *json, rsa_context *rsa){
    ...
    while(json != NULL) {
      if (strcmp(json->name, "commonName")==0)
        oid = OID_CN;
      else if(strcmp(json->name, "orgunit")==0)
        oid = OID_ORG_UNIT;
      else if(strcmp(json->name, "state")==0)
        oid = OID_STATE;
      else if (strcmp(json->name, "country")==0)
        oid = OID_COUNTRY;
      else if (strcmp(json->name, "orgname")==0)
        oid = OID_ORGANIZATION;
      else if (strcmp(json->name, "city")==0)
        oid = OID_LOCALITY;
      else if (strcmp(json->name, "email")==0)
        oid = OID_PKCS9_EMAIL;
